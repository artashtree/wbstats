...
const transformXmlDataToArray = (str) => {
    ...
    for (let i = 0; i < records.length; i++) {
        try {
            const newItem = {};
            const indicator =
                records[i].getElementsByTagName('wb:indicator')[0].id;
            const country =
                records[i].getElementsByTagName('wb:country')[0].innerHTML;
            const iso3code =
                records[i].getElementsByTagName('wb:countryiso3code')[0]
                    .innerHTML;
            const value =
                records[i].getElementsByTagName('wb:value')[0].innerHTML;
            const year =
                records[i].getElementsByTagName('wb:date')[0].innerHTML;
            newItem.year = year;
